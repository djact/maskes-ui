...
const Comment = (props) => {
	...
	const toggleEditor = (i) => {
		setCms(
			cms.map((cm, j) =>
				j === i ? { ...cm, key: j, onEdit: !cm.onEdit } : { ...cm, key: j }
			)
		);
	};
	...
	const toggleColapsed = (i) => {
		setCms(
			cms.map((cm, j) =>
				j === i
					? { ...cm, key: j, collapsed: !cm.collapsed }
					: { ...cm, key: j }
			)
		);
	};
	...
	if (!loading && comments && comments.results.length > 0) {
		display = cms.map((comment, index) => (
			<CommentSUI key={comment.id}>
				...
				<CommentSUI.Content>
					...
					{!comment.onEdit ? (
							...
							<CommentSUI.Actions>
								<CommentSUI.Action onClick={(e) => toggleColapsed(index)}>
									<CommentSUI.Metadata>
										...
										{comment.collapsed ? null : <FiChevronDown />}
									</CommentSUI.Metadata>
								</CommentSUI.Action>
								{userId === comment.author ? (
									<CommentSUI.Action
										className="edit-btn"
										onClick={(e) => toggleEditor(index)}
									>
										{edit_button}
									</CommentSUI.Action>
								) : null}
						...
						<Form
							reply
							onSubmit={() => update(comment.id, comment.comment_content)}
						>
							<Form.TextArea
		...
		display = <div>No comment yet</div>;
	}
