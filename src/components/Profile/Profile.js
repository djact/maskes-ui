...
const Profile = (props) => {
	...
	const edit_button = (toggleEdit) =>
		isOwner && (
			<Button variant="link" className="edit-btn" onClick={toggleEdit}>
				<BsPencil />
			</Button>
		);
	...
	const save_cancel_icons = (toggleEdit) =>
		isOwner && (
			<div className="m-2">
				<Button variant="link" className="edit-info-btn" type="submit">
					<BsCheck />
				</Button>
				<Button variant="link" className="delete-info-btn" onClick={toggleEdit}>
					<BsX />
				</Button>
			</div>
		);
	...
	const toggleProfileEditor = (i) => {
		setProfileData(
			profileData.map((pd, j) =>
				j === i ? { ...pd, key: j, onEdit: !pd.onEdit } : { ...pd, key: j }
			)
		);
	};
	...
	const onChangeProfileData = (e, i) => {
		setProfileData(
			profileData.map((pd, j) =>
				i === j
					? { ...pd, key: j, [e.target.id]: e.target.value }
					: { ...pd, key: j }
			)
		);
	};
	...
	const onChangePrivacy = (e, i) => {
		...
		setPrivacy(
			privacy.map((pvc, j) =>
				j === i
					? { ...pvc, key: j, [e.target.id]: e.target.checked }
					: { ...pvc, key: j }
			)
		);
		axios.put(url, body);
										...
										<Col className="text-secondary mb-1" md="auto">
											{
												<Button
													...
													onClick={
														isOwner
															? () => toggleProfileEditor(3)
															: () =>
																	window.open(
																		`http://www.faceboook.com/${profile.facebook}`,
																		'_blank'
																	)
													}
												>
										...
										<Col className="text-secondary  mb-1" md="auto">
											{
												<Button
													...
													onClick={
														isOwner
															? () => toggleProfileEditor(5)
															: () =>
																	window.open(
																		`https://venmo.com/${profile.venmo}`,
																		'_blank'
																	)
													}
												>
					...
					<Card className="mb-3">
						<Card.Body>
							{showEditFullName ? (
								...
								<Row>
									...
									<Col sm={6} className="text-secondary">
										{!profile.fullname_privacy || isOwner
									...
									{isOwner && (
										<Col
											sm="auto"
											className="text-secondary my-auto pull-right"
										>
											<Form>
												<Form.Check
													style={{
														color: privacy[0].fullname_privacy ? 'teal' : null
													}}
													type="switch"
													id="fullname_privacy"
													label={'privacy'}
							...
							<Row>
								...
								{isOwner && (
									<Col sm="auto" className="text-secondary my-auto">
										<Form>
											<Form.Check
												style={{
													color: privacy[1].email_privacy ? 'teal' : null
												}}
												type="switch"
												id="email_privacy"
												label={'privacy'}
									...
									<Col sm="auto" className="text-secondary my-auto">
										<Form>
											<Form.Check
												type="switch"
												style={{
													color: privacy[2].phone_privacy ? 'teal' : null
												}}
												id="phone_privacy"
												label={'privacy'}
									...
									<Col sm="auto" className="text-secondary my-auto">
										<Form>
											<Form.Check
												type="switch"
												style={{
													color: privacy[3].location_privacy ? 'teal' : null
												}}
												id="location_privacy"
												label={'privacy'}
							...
							<Card.Body>
								<Row>
									<Button
										className="mb-0"
										variant="link"
										onClick={() => history.push('/email-reset')}
									>
										Reset Email
									</Button>
								</Row>
					...
					<Row className="gutters-sm">
							...
							<Card className="h-100">
								<Card.Body>
									<h6 className="d-flex align-items-center mb-3">
										<i className="material-icons text-info mr-2">Volunteer</i>
										{profile.volunteer_count}
									</h6>
								</Card.Body>
							</Card>
						</Col>
						<Col sm={6} className="mb-3">
							<Card className="h-100">
								<Card.Body>
									<h6 className="d-flex align-items-center mb-3">
										<i className="material-icons text-info mr-2">Delivered</i>
...
Profile.propTypes = {
	profile: PropTypes.shape({
		...
		venmo: PropTypes.string,
		email_privacy: PropTypes.bool,
		fullname_privacy: PropTypes.bool,
		location_privacy: PropTypes.bool,
		phone_privacy: PropTypes.bool,
		id: PropTypes.number,
		user: PropTypes.number,
		volunteer_count: PropTypes.number,
		delivered_count: PropTypes.number
	}),
	myId: PropTypes.string,
	nameChangeSubmitHandler: PropTypes.func,
