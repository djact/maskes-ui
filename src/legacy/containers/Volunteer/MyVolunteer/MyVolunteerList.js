...
const MyVolunteerList = (props) => {
    const { volunteer, loading, name, fetchVolunteerList, status } = props
    ...
    if (!loading && results && results.length > 0) {
        display = results.map((volunteer, index) => (
            ...
            >
                <Card.Body>
                        ...
                        <Card className="request_shadow">
                            ...
                            <Accordion.Collapse eventKey="1">
                                <Card.Body>
                                    <Card.Text>
                                        Location:{' '}
                                        {volunteer.request_info.locations}
                                    </Card.Text>
                                    <Card.Text>
                                        Family size:{' '}
                                        {
                                            volunteer.request_info
                                                .household_number
                                        }
                                    </Card.Text>
                                    <Card.Text>
                                        Urgent Needs:{' '}
                                        {volunteer.request_info.urgency}
                                    </Card.Text>
                                    <Card.Text>
                                        Request Date:{' '}
                                        {new Date(
                                            volunteer.request_info.created_date
                                        ).toLocaleDateString()}
                                    </Card.Text>
                                </Card.Body>
                            </Accordion.Collapse>
                    ...
                    <Card.Text>
                        Date:{' '}
                        {new Date(volunteer.created_date).toLocaleDateString()}
                    </Card.Text>
                    <Button
                        size="sm"
                        variant="link"
                        ...
                        onClick={() =>
                            history.push(`my-support/${volunteer.id}`)
                        }
                    >
                        {volunteer.status === 'Signed Up'
                            ? 'Waiting for admin...'
        ...
        display = <p>You haven't signed up for any volunteer yet!</p>
    }
